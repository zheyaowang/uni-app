<template>
	<view class="detail-crop-growth height_full flex flex-direction bg-white">
		<sk-navbar :title="navbarTitle"></sk-navbar>

		<view class="detail-crop-growth__head">
			<u--text color="#fff" blod size="32rpx" text="透光度"></u--text>
			<view class="flex align-center" style="margin: 35rpx 0">
				<u--image width="60rpx" height="56rpx" src="/static/img/work-space/detail/head_crop_icon.png"></u--image>
				<u--text color="#fff" margin="0 20rpx" blod size="56rpx" :text="`${monitorValue || ' - '}`"></u--text>
			</view>
			<u--text color="#fff" align="right" margin="8rpx 0 0" blod size="26rpx" :text="monitorTime"></u--text>
		</view>

		<view class="detail-crop-growth__content flex flex-direction flex-1">
			<view class="form-content flex align-center justify-between">
				<u--text color="#666" margin="20rpx 0" size="28rpx" text="生长进度"></u--text>
	...
	methods: {
		async getMonitorInfo() {
			try {
				...
				const response = await apiServer.getCropGrowthInfoReportApi({ stationGuid, startTime, endTime });
				if (response.code === 200) {
					const { leafLightTransmittanceRT, tm } = response.data;
					this.monitorValue = leafLightTransmittanceRT;
					this.monitorTime = timeFormat(new Date(tm), 'yyyy-MM-dd HH:mm');
					this.$nextTick(() => {
						// 生成charts
		...
		createCharts({ dataList }) {
			...
			for (const item of dataList) {
				const { tm, leafLightTransmittance } = item;
				categories.push(timeFormat(new Date(tm), 'MM-dd'));
				seriesData.push(leafLightTransmittance);
			}
			this.chartData = {
				categories,
				series: [
