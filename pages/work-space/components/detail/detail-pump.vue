<template>
	<view class="detail-pump height_full">
...
export default {
	...
	methods: {
		async getMonitorInfo() {
			try {
				// 获取一体泵详情
				const response = await apiServer.getPromonitorPumpDetailApi(this.id);
				if (response.code === 200) {
					this.equimentInfo = response.data;
				}
			} catch (e) {
				//TODO handle the exception
				console.error('获取一体泵详情', e);
			}
		},
		async getMonitorData() {
			try {
				...
				const response = await apiServer.getPromonitorPumpDataApi({ id, startTime, endTime });
				if (response.code === 200) {
					this.equimentData = response.data;
				}
			} catch (e) {
				//TODO handle the exception
				console.error('获取一体泵数据', e);
			}
		}
	}
};
