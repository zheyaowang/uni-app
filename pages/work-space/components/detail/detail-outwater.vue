<template>
	<view class="detail-outwater height_full">
...
export default {
	...
	methods: {
		async getMonitorInfo() {
			try {
				// 获取出水闸详情
				const response = await apiServer.gettPromonitorOutletsluiceDetailApi(this.id);
				if (response.code === 200) {
					this.equimentInfo = response.data;
				}
			} catch (e) {
				//TODO handle the exception
				console.error('获取出水闸详情', e);
			}
		},
		async getMonitorData() {
			try {
				...
				const response = await apiServer.gettPromonitorOutletsluiceDataApi({ id, startTime, endTime });
				if (response.code === 200) {
						this.equimentData = response.data;
				}
			} catch (e) {
				//TODO handle the exception
				console.error('获取出水闸数据', e);
			}
		},
	}
};
