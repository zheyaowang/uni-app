			...
			<view class="u-tabs__wrapper__scroll-view-wrapper">
				...
				>
					...
					>
						...
						>
							<view @click="activeClick(index)" style="display: flex; flex-direction: column;align-items: center; padding: 8px;background-color: #fff;
								box-shadow: 0 2px 4px rgba(0, 0, 0, .12), 0 0 6px rgba(0, 0, 0, .04);border-radius: 4px;">
								<image style="width: 30px;height: 30px; " :src="active === index ? item.selectedIconPath : item.iconPath"></image>
		...
		methods: {
			...
			resize() {
				...
				Promise.all([this.getTabsRect(), this.getAllItemRect()]).then(([tabsRect, itemRect = []]) => {
					...
					// 组件在swiper下面的current !== 0 的时候，初始化进来位置不对的情况，需要减去N x 屏幕宽度
					let windowWidth
					uni.getSystemInfo({
						success(res) {
							windowWidth =  res.windowWidth * Number(this.pageIndex)
						}
					})
					if(this.isFrist && this.handleWidth) {
						tabsRect.left = tabsRect.left - windowWidth
						tabsRect.right = tabsRect.right - windowWidth
					}
					itemRect.map((item, index) => {
						if(this.isFrist && this.handleWidth) {
							item.left = item.left - windowWidth
							item.right = item.right - windowWidth
						}
						// 计算scroll-view的宽度，这里
