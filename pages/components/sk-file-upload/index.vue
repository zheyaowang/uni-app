		...
		methods: {
			...
			videoAdd() {
				...
				uni.chooseVideo({
					...
					success: (res) => {
						...
						this.chooseSuccessMethod([res.tempFilePath], 1, new Array(res.tempFile))
			...
			imgAdd() {
				...
				uni.chooseImage({
					...
					success: (res) => {
						console.log('res', res)
						this.chooseSuccessMethod(res.tempFilePaths, 0, res.tempFiles)
						//console.log('tempFiles', res)
						// if (this.action == '') { //未配置上传路径
						// 	this.$emit("chooseSuccess", res.tempFilePaths);
						// } else {
			...
			// filePath文件二进制路径， type类型， tempFiles文件全信息
			chooseSuccessMethod(filePaths, type, tempFiles) {
				if (this.action === '') { //未配置上传路径
					this.uploadFilePromise(filePaths, type, tempFiles)
					this.$emit("chooseSuccess", filePaths, type, tempFiles); //filePaths 路径 type 0 为图片 1为视频
				} else {
					if (type == 1) {
			...
			// 手动控制上传
			async uploadFilePromise(filePaths,type, tempFiles) {	
				...
				const tasks = tempFiles.map((item, index) => {
					const identifier = this.guid()
					const data = {
						chunkNumber: 1,
						chunkSize: 152428800,
					...
					return new Promise(async (resolve, reject) => {
						...
						uni.uploadFile({
							...
							header: {
								// 'Authorization': uni.getStorageSync(TOKEN_NAME),
								// 'Content-Type': 'application/x-www-form-urlencoded',
								// 'Cookie': 'ASP.NET_SessionId=ck5e24sihorysim5zo1olfqm',
							...
							success: async (uploadFileRes) => {
								...
								// uni.uploadFile({
								// 	url: `${configService.apiUrl}/file/merge`,
								// 	filePath: item.path,
								// 	// header: this.headers,
								// 	formData: params,
								// 	name: this.name,
								// 	success: (result) => {
								// 		// 
								// 		// resolve({...result.data, fileId: result.data.name, url: result.data.url, }) 
								// 		result.code === 200 ? resolve({...result.data,fileId: result.data.name, url: result.data.url, }) : reject((result.data))
								// 	}
								// })
								...
								? resolve({
									...result.data,
									fileId: result.data.name, // 搞不懂这里为啥传这么多重复的值，一个上传接口、口区
									url: result.data.url, // fullUrl =  configService.fileUrl + 
								}) 
								: reject((result.data))
							},
							fail: (err) => {
				...
				Promise.all(tasks).then(res => {
					...
					uni.hideLoading();
					res.forEach(item => {
						this.uploadLists.push(item.url)
					})
					console.log('this.uploadLists', this.uploadLists);
