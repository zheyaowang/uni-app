<template>
	<view class="seck-flex-y height_full">
		...
		<view class="search-bar">
			<!-- <u--text text="1#田于2023-04-14至2023-05-01期间用水总量：5265m3" lines="2" size="13" color="#666"></u--text> -->
			<view v-if="range.length">
				<u--text :text="proName + ' 于 ' + range[0] + '至' + range[1] + '期间用水总量：'" lines="2" size="13" color="#666"></u--text>	
			</view>
			<view v-else>
				<u--text :text="proName + '用水总量：'" lines="2" size="13" color="#666"></u--text>	
			</view>
			<view class="seck-flex-y-center gap-xs">
				<u--text :text="useWaterTotal" lines="1" size="18" color="#13C110" lineHeight="20" style="white-space: nowrap;"></u--text>
				<u--text text="m³" lines="1" size="13" color="#666" style="white-space: nowrap;"></u--text>
			</view>
		</view>
		<view>
			<!-- 不显示input，调用api打开 -->
		...
		<view class="seck-flex-fill seck-flex-y margin-top-sm height_full">
			<scroll-view style="height: 100%;" scroll-top="0" enableBackToTop="true" scroll-y>
				<uni-table ref="table" :loading="loading" :border="false" stripe emptyText="暂无更多数据">
					<uni-tr>
						<uni-th width="190rpx" align="center">用水时间</uni-th>
						<uni-th width="190rpx" align="center">田地编号</uni-th>
						<uni-th width="190rpx" align="center">农户名称</uni-th>
						<uni-th width="190rpx" align="center">用水量(m³)</uni-th>
					</uni-tr>
					<uni-tr v-for="(item, index) in tableData" :key="index">
						...
						<uni-td align="center">{{ item.responsiblePersonName }}</uni-td>
						<uni-td align="center">
...
<script>
	...
	import { getProjectWateruseDetails } from '@/api/water-fertilizer.js'
	...
	export default {
		...
		onLoad() {
			...
			eventChannel.on('acceptDataFromOpenerPage', (data) => {
				console.log(data)
				this.navbarTitle = data.proName + ' 用水详情'
				...
				eventChannel.emit('someEvent', {
					data: 'data form use water details'
				})
			})
		},
		mounted() {
		...
		methods: {
			...
			async getTableData() {
				...
				try{
					const { data } = await getProjectWateruseDetails(params, true)
					console.log('data', data);
					this.useWaterTotal = 0
					// 没数据的情况
					if(!data.list.length) {
						this.tableData = []
						return false
					...
					this.useWaterTotal = sum(data.list, 'consumptionValue')
				}catch(e){
