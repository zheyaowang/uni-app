<template>
	<view class="seck-flex-y height_full">
		...
		<view class="search-bar">
			<!-- <u--text text="1#田于2023-04-14至2023-05-01期间用药总量：5265m3" lines="2" size="13" color="#666"></u--text> -->
			<view v-if="range.length">
				<u--text :text="proName + ' 于 ' + range[0] + '至' + range[1] + '期间用药总量：'" lines="2" size="13" color="#666"></u--text>	
			</view>
			<view v-else>
				<u--text :text="proName + '用药总量：'" lines="2" size="13" color="#666"></u--text>	
			</view>
			<view class="seck-flex-y-center gap-xs">
				<u--text :text="usePesticideTotal" lines="1" size="18" color="#13C110" lineHeight="20" style="white-space: nowrap;"></u--text>
		...
		<view class="seck-flex-fill seck-flex-y margin-top-sm" style="height: 0;">
			<scroll-view style="height: 100%;" scroll-top="0" enableBackToTop="true" scroll-y>
				<uni-table ref="table" :loading="loading" :border="false" stripe emptyText="暂无更多数据">
					<uni-tr>
						<uni-th width="190rpx" align="center">用药时间</uni-th>
						<uni-th width="190rpx" align="center">田地编号</uni-th>
						<uni-th width="190rpx" align="center">用药类型</uni-th>
						<uni-th width="190rpx" align="center">用药量(吨)</uni-th>
					</uni-tr>
...
<script>
	...
	import { getProjectPharmacyDetails } from '@/api/water-fertilizer.js'
	...
	export default {
		...
		onLoad() {
			...
			eventChannel.on('acceptDataFromOpenerPage', (data) => {
				...
				this.navbarTitle = data.proName + ' 用药详情'
				...
				eventChannel.emit('someEvent', {
					data: 'data form use pesticide details'
				})
			})
		},
		mounted() {
		...
		methods: {
			...
			async getTableData() {
				...
				try{
					const { data } = await getProjectPharmacyDetails(params, true)
					console.log('data', data);
					this.usePesticideTotal = 0
					// 没数据的情况
					if(!data.list.length) {
						this.tableData = []
						return false
					...
					this.usePesticideTotal = sum(data.list, 'consumptionValue')
				}catch(e){
