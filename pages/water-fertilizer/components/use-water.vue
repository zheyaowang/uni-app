<template>
	<view class="height_full seck-flex-y height_full">
		...
		<view class="search-bar seck-flex-column width_full">
			...
			<view class="seck-flex-y-center width_full gap">
				...
				<view>
					<sk-select
						...
						style="width: 170rpx;background-color: #fff;"
						placeholder="田地编号"
					></sk-select>
				</view>
			...
			<view class="" style="width: 100%;display: flex;height: 40rpx;line-height: 40rpx;margin-top: 10rpx">
				<view><u--text text="用水总量:" size="20rpx" color="#666"></u--text></view>
				<view style="margin: 0 10rpx"><u--text :text="useWaterTotal" size="20rpx" color="#13C110"></u--text></view>
				<view><u--text text="m³" size="20rpx" color="#666"></u--text></view>
			</view>
		</view>
...
import { getProjectWateruse } from '@/api/water-fertilizer.js';
...
export default {
	...
	methods: {
		...
		async getMessageData() {
			...
			try {
				const { data } = await getProjectWateruse(params, true);
				...
				for (let i = 0; i < data.length; i++) {
					...
					this.msgList =
						...
						data[i].detailList.map(item => {
							this.useWaterTotal += item.consumptionValue; // 累加用水总量
							return {
								...item,
		...
		goDetails(item) {
			uni.navigateTo({
				url: '/pages/water-fertilizer/components/use-water-details',
				events: {
...
/deep/ .uni-date {
	.uni-date__x-input,  .range-separator{
		height: 58rpx;
		line-height: 58rpx;
	}
	.uni-icons {
