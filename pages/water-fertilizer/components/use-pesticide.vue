	...
	<view class="water-fertilizer seck-flex-y height_full">
		...
		<view class="search-bar seck-flex-column width_full">
			...
			<view class="" style="width: 100%;display: flex;height: 40rpx;line-height: 40rpx;margin-top: 10rpx">
				<view><u--text text="用药总量:" size="20rpx" color="#666"></u--text></view>
				<view style="margin: 0 10rpx"><u--text :text="usePesticideTotal" size="20rpx" color="#13C110"></u--text></view>
					...
					<view style="display: flex;flex-direction: row; align-items: center;" @tap="$u.debounce(() => goDetails(item), 300)">
						...
						>
							...
							<view class="padding-top-sm seck-flex">
								<view><u--text lines="1" text="用药量:"></u--text></view>
								<view>
									<!-- 数字需要调下行高 -->
...
import { getProjectPharmacy } from '@/api/water-fertilizer.js';
...
export default {
	...
	data() {
		return {
			...
			householdHeadNameOptions: [], // 户主姓名选项
			paddyFieldNumberOptions: [], // 土地编号选项
	...
	methods: {
		...
		async getMessageData() {
			...
			try {
				const { data } = await getProjectPharmacy(params, true);
				...
				for (let i = 0; i < data.length; i++) {
					...
					this.msgList =
						...
						data[i].detailList.map(item => {
							this.usePesticideTotal += item.consumptionValue; // 累加用药总量
							return {
								...item,
		...
		goDetails(item) {
			uni.navigateTo({
				url: '/pages/water-fertilizer/components/use-pesticide-details',
				events: {
