<template>
	<view class="u-switch" :class="[disabled && 'u-switch--disabled']" :style="[switchStyle, $u.addStyle(customStyle)]" @tap="clickHandler">
		...
		<text></text>
		<view class="u-switch__label" :style="labelStyle" v-if="showLabel">
			<u--text
				:text="value ? labelList[0] : labelList[1]"
				:align="value ? 'left' : 'right'"
				:color="value ? '#fff' : '#AAABAD'"
				:size="size * (labelList[0].length > 1 ? 0.4 : 0.5)"
			></u--text>
		</view>
	</view>
</template>
...
export default {
	...
	computed: {
		...
		switchStyle() {
			let style = {};
			// 展示文字，增加宽度
			const number = this.showLabel ? (this.labelList[0].length > 1 ? 8 : 0) : 0;
			// 这里需要加2，是为了腾出边框的距离，否则圆点node会和外边框紧贴在一起
			style.width = uni.$u.addUnit(this.size * 2 + 2 + number);
		...
		nodeStyle() {
			...
			// 展示文字，增加宽度
			const number = this.showLabel ? (this.labelList[0].length > 1 ? 8 : 0) : 0;
			const translateX = this.isActive ? uni.$u.addUnit(this.space) : uni.$u.addUnit(this.size + number);
